<script setup lang="ts">
    ...
    const { data, error } = await useFetch<RecipeResponse>("https://dummyjson.com/recipes?limit=21");
            ...
            <div class="container flex flex-col lg:flex-row items-center py-20 gap-10">
                <div class="flex-1 order-2 lg:order-1 text-center lg:text-left">
                    <h1 class="text-4xl lg:text-6xl font-extrabold mb-6 text-balance">
                        Master the Kitchen with Ease: Unleash Your Chef Today!                            
                    </h1>
                    <p class="text-xl lg:text-2xl mb-8 text-balance">
                        Discovery recipes helping you to find the easiest way to cook.
                    </p>
                    <button class="px-4 py-2 text-white self-start bg-dodroll-gold rounded-md text-lg cursor-pointer">
                        Browse Recipes
                    </button>
                </div>
                <div class="flex-1 order-1 lg:order-2">
                    <NuxtImg sizes="xs:100vw sm:100vw md:100vw lg:50vw" src="/nuxt-course-hero.png" densities="1x" alt="" />
                </div>
            </div>
        </section>
