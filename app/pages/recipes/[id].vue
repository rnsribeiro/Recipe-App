...
<template>
   <div class="flex flex-col max-w-screen-lg container py-20">
    ...
    <NuxtImg
      ...
      class="w-full max-h-[500px] object-cover rounded-mc shadow-sm mb-12" 
      alt=""
    />
        ...
        <ul class="grid grid-cols-1 md:grid-cols-2 gap-2 text-lg">
            <li v-for="ingredient in data?.ingredients">
                <label class="flex gap-2 items-center">
                    ...
                    <div class="relative w-6 h-6 rounded-full border-2 border-dodroll-gold flex items-center justify-center 
                    peer-checked:after:absolute peer-checked:after:w-4 peer-checked:after:h-4 peer-checked:after:bg-dodroll-gold 
                    peer-checked:after:rounded-full"></div>
                    <span class="peer-checked:line-through peer-checked:text-gray-500">{{ ingredient }}</span>
                </label>
            </li>
        </ul>
    </div>
    ...
    <div>
        ...
        <ul class="flex flex-col text-lg gap-4">
            <li v-for="(instruction, index) in data?.instructions" class="flex gap-2">
                <span class="flex items-center justify-center bg-dodroll-gold w-7 h-7 rounded-full text-white text-sm">
                    {{ index + 1 }}
                </span>
